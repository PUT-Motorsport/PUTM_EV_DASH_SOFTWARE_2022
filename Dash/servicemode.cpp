...
ServiceMode::ServiceMode(QWidget *parent)
    : QDialog(parent), ui(new Ui::ServiceMode), subwindowShown(nullptr) {
  ui->setupUi(this);
  driving = new DrivingSelect();
  canRaw = new CanRaw();
  logs = new Logs();

  QObject::connect(driving, &DrivingSelect::finished, this,
                   &ServiceMode::reopen);
...
void ServiceMode::updateData(Parameter param, int value) {
