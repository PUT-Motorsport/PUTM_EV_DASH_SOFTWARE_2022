// Generated on Wed Jun 15 10:27:26 2022
#ifndef DASH
#define DASH

#include <cstdint>

enum struct Dash_states : uint8_t {
  Power_up,
  Normal_operation,
  Driverless,
};

struct __attribute__((packed)) Dash_Main {
  Dash_states device_state;
};

struct __attribute__((packed)) Dash_TCS {
  uint8_t on;              // TC- ON - OFF
  uint8_t max_slip_ratio;  // 0-100%
  uint8_t algorithm;       // "PI (PID
  uint8_t max_power;       // 0-100%
  uint8_t apps_curve;      // "0 - Linear
  uint8_t regen_power;     // 0-100%
  uint8_t sensivity;       // 0-100% [100% - no wheel slip]
  uint8_t CRC_byte;
};

struct __attribute__((packed)) Dash_steering_wheel_angle {
  uint16_t steering_wheel_angle;  // Steering wheel angle [0-360] expressed in
                                  // 11 bits [0-4096]
};

struct __attribute__((packed)) __attribute__((deprecated))
Dash_Smart_Fuses_FAN_speed {
  uint8_t fan_2;       // 0-100% []
  uint8_t water_pump;  // on off
};

struct __attribute__((packed)) __attribute__((deprecated))
Dash_steering_wheel_request {};

struct __attribute__((packed)) __attribute__((deprecated)) Dash_lap_finished {
  uint32_t time;  // total lap time (all sectors) in microseconds
};

const uint16_t DASH_MAIN_CAN_ID = 0x64;
const uint8_t DASH_MAIN_CAN_DLC = sizeof(Dash_Main);
const uint8_t DASH_MAIN_FREQUENCY = 10;
const uint16_t DASH_TCS_CAN_ID = 0x19;
const uint8_t DASH_TCS_CAN_DLC = sizeof(Dash_TCS);
const uint8_t DASH_TCS_FREQUENCY = 0;
const uint16_t DASH_STEERING_WHEEL_ANGLE_CAN_ID = 0x8c;
const uint8_t DASH_STEERING_WHEEL_ANGLE_CAN_DLC =
    sizeof(Dash_steering_wheel_angle);
const uint8_t DASH_STEERING_WHEEL_ANGLE_FREQUENCY = 50;
[[deprecated]] const uint16_t DASH_SMART_FUSES_FAN_SPEED_CAN_ID = 0x32;
[[deprecated]] const uint8_t DASH_SMART_FUSES_FAN_SPEED_CAN_DLC =
    sizeof(Dash_Smart_Fuses_FAN_speed);
[[deprecated]] const uint8_t DASH_SMART_FUSES_FAN_SPEED_FREQUENCY = 0;
[[deprecated]] const uint16_t DASH_STEERING_WHEEL_REQUEST_CAN_ID = 0x37;
[[deprecated]] const uint8_t DASH_STEERING_WHEEL_REQUEST_CAN_DLC =
    sizeof(Dash_steering_wheel_request);
[[deprecated]] const uint8_t DASH_STEERING_WHEEL_REQUEST_FREQUENCY = 0;
[[deprecated]] const uint16_t DASH_LAP_FINISHED_CAN_ID = 0x7d;
[[deprecated]] const uint8_t DASH_LAP_FINISHED_CAN_DLC =
    sizeof(Dash_lap_finished);
[[deprecated]] const uint8_t DASH_LAP_FINISHED_FREQUENCY = 0;

#endif
